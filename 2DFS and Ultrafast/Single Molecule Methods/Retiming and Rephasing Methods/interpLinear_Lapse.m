...
function[XnLin, YnLin, Xshifts, Yshifts, Xangles, Yangles, tnX, tnY, tx, ty, NIFFT,Xzero] = interpLinear_Lapse(plotOpt,XLin, YLin, tx, ty, Tdes,scanNum,photGrid,printOpt) 
...
% the photGrid is 10x9, so that the X scans are the columns and the Y scans
% are the rows 
xPhots= sum(photGrid(:,scanNum));
yPhots= sum(photGrid(scanNum,:)); 

% convert the number of photons to a string which can be used as scientific
% notation
powTenX= numel(num2str(xPhots))-1;
redPhotX= xPhots/(10^powTenX);
photStrX= num2str(redPhotX);
photLabelX = [' N=' photStrX(1:5) ,'E',num2str(powTenX)];

% convert the number of photons to a string which can be used as scientific
% notation
powTenY= numel(num2str(yPhots))-1;
redPhotY= yPhots/(10^powTenY);
photStrY= num2str(redPhotY);
photLabelY = [' N=' photStrY(1:5) ,'E',num2str(powTenY)]; 

if plotOpt 
figure(11)
hold on
plot(1e-3*(Faxis+Fmono), abs(YnFLin(:,scanNum)),'LineWidth',2.0);
plot(1e-3*(Faxis+Fmono), real(YnFLin(:,scanNum)),'LineWidth',2.0);
plot(1e-3*(Faxis+Fmono), imag(YnFLin(:,scanNum)),'LineWidth',2.0);
title(['Y Linear Abs Retimed ' photLabelY]);
xlabel('Wavenumbers (x10^3 cm^-^1)');
ylabel('Magnitude (arb)'); 
xlim([17 20.5]); 
legend('Abs', 'Real','Imag'); 
if printOpt
    print -dpdf -r300 -bestfit YlinAbs.pdf
end 

figure(12)
hold on
plot(1e-3*(Faxis+Fmono), abs(XnFLin(scanNum,:).'),'LineWidth',2.0);
plot(1e-3*(Faxis+Fmono), real(XnFLin(scanNum,:).'),'LineWidth',2.0);
plot(1e-3*(Faxis+Fmono), imag(XnFLin(scanNum,:).'),'LineWidth',2.0);
% plot(f, real(SFLin.'), f, imag(SFLin.'), f, abs(SFLin.'));
title(['X Linear Abs Retimed ' photLabelX]);
xlabel('Wavenumbers (x10^3 cm^-^1)');
ylabel('Magnitude (arb)'); 
xlim([17 20.5]); 
legend('Abs', 'Real','Imag');
if printOpt
    print -dpdf -r300 -bestfit XlinAbs.pdf
end 
end 
