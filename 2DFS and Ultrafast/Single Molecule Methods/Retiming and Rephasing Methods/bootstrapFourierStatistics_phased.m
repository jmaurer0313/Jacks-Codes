...
function [xerr, yerr, abserr, phierr, rawbootstrap, fftbootstrap] ...
    = bootstrapFourierStatistics_phased(nboot, nfreq, data, phase)

    %rawbootstrap is a nboot*length(data) matrix
    N = length(data);
    rawbootstrap = bootstrp(nboot, @(x) x, data);
    fftbootstrapX = zeros(nboot, nfreq+1);
    fftbootstrapY = zeros(nboot, nfreq+1);
    fftbootstrap = zeros(nboot, nfreq+1);
    for k = 0:(nfreq)
        %Calculate the kth fourier component of the data
        fftbootstrapX(:,k+1) = (1/N * sum(exp(-1i*k*rawbootstrap),2))*exp(1i*phase);
        fftbootstrapY(:,k+1) = (1/N * sum(exp(-1i*k*rawbootstrap),2))*exp(1i*(pi-phase));
        fftbootstrap(:,k+1) = 1/N * sum(exp(-1i*k*rawbootstrap),2);
    end
    xerr = std(real(fftbootstrapX),0,1);
    yerr = std(imag(fftbootstrapY),0,1);
