...
base_folder = 'C:\Users\Jack\Documents\Marcus Lab\Data\2DFS\Cy3DNA_Bulk\20211005\20211005-162036';
...
window2steep = delayedGaussian(sqrt(wt21.^2 + wt43.^2), 60, 3);
...
% dmatWPyFT2(:,i)=(dmatWPyFT2(:,i)).*FFTwindowy.';
% smatWPyFT2(:,i)=(smatWPyFT2(:,i)).*FFTwindowYs.';
% end
...
for i=1:size(smatIntPx2,1)
    if (avgShiftI)~=0
    smatIntPx2(i,(1:avgShiftI-1))=0; 
    smatIntPx2(i,:)=circshift((smatIntPx2(i,:)),-(avgShiftI-1),2);
%     smatIntPx2(i,:)=circshift((smatIntPx2(i,:)),-(t21shifts(i,2)),2);  

    
    dmatIntPx2(i,(1:avgShiftI-1))=0; 
    dmatIntPx2(i,:)=circshift((dmatIntPx2(i,:)),-(avgShiftI-1),2);  
    end
end
...
window2Dfinal = delayedGaussian(sqrt(wXn2.^2 + wYn2.^2), 50, 15);
