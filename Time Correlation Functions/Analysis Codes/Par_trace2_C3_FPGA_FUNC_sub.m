...
% Program information
%--------------------------------------------------------------------------
function [totalRunTime,avgFileRunTime,tau1L,tau3L,NfilesToIterate,filesList] = Par_trace2_C3_FPGA_FUNC_sub(usres,constructName,tau2ArrayUnits_us,subsetOpt,subsetLoc,subsetName,quantityName,calcZeroTau) 
...
%     all based on starting at 10usec resolution for the stitching...
   ...
   elseif usres==50
    ...
    tauArray_msec = tauArray_msec(tauArray_msec>=1000);
    tauArray_msec = (unique(tauArray_msec/1000))*1000;
    tauArrayUsecTotal = [50, linspace(100,1000,19),(tauArray_msec)']';
    tauArrayTotal = unique(floor(tauArrayUsecTotal/(10)));
    tauArrayUsecTotal=tauArrayTotal*10;
    tauArraySecTotal= tauArrayUsecTotal*1e-5;
       
       tau1L=tauArrayUsecTotal(tauArrayUsecTotal>=50);
       tau3L=tauArrayUsecTotal(tauArrayUsecTotal>=50);      
   ...
   elseif usres==250
       ...
       tauArray_msec = tauArray_msec(tauArray_msec>=1000);
       tauArray_msec = (unique(tauArray_msec/1000))*1000;
       tauArrayUsecTotal = [250 500 750 (tauArray_msec)']';
       
       tau1S=tauArrayUsecTotal(tauArrayUsecTotal>=250);
       tau1S=tau1S(tau1S<=1000);
       tau3S=tauArrayUsecTotal(tauArrayUsecTotal>=250);
       tau3S=tau3S(tau3S<=1000);
       tau1L=tauArrayUsecTotal(tauArrayUsecTotal>=250);
       tau3L=tauArrayUsecTotal(tauArrayUsecTotal>=250);  
    ...
    elseif usres==500
       ...
       tauArray_msec = tauArray_msec(tauArray_msec>=1000);
       tauArray_msec = (unique(tauArray_msec/1000))*1000;
       tauArrayUsecTotal = [500 (tauArray_msec)']';
       
       tau1S=[];
       tau3S=[];
       tau1L=tauArrayUsecTotal(tauArrayUsecTotal>=500);
       tau3L=tauArrayUsecTotal(tauArrayUsecTotal>=500);
   ...
   elseif usres==1000
    ...
    elseif usres==5000
       ...
       NptsToAdd = NptsTCFDesired - 200;
       tauArray_msec = ([1:9, round(logspace(1, 3,NptsToAdd))]')*1000;
       tauArray_msec = tauArray_msec(tauArray_msec>=5000);
       tauArray_msec = (unique(round(tauArray_msec/5000)))*5000;
       tauArrayUsecTotal = [(tauArray_msec)']';
       
       tau1S=[];
       tau3S=[];
       tau1L=tauArrayUsecTotal(tauArrayUsecTotal>=5000);
       tau3L=tauArrayUsecTotal(tauArrayUsecTotal>=5000);
...
fprintf(['The dimensions of the 3pt will be tau1 = %d:%d usec (%d pts)' ...
...
%         client, possibly leading to signifcant overhead in parallel. 
        if usres==1000 || usres==10000 || usres==500 || usres==5000 || usres==50
        tau1LUnits = tau1L/usres;
        tau3LUnits = tau3L/usres;
        elseif usres==10 || usres==100 || usres==250
        tau1LUnits = tau1L/usres;
        tau3LUnits = tau3L/usres;
...
%         step = 0;
            ...
            elseif strcmp(quantityName,'Rate')
            f = curFile.counts;
            f = abs(f./(usres*1e-6));
            fAvg = nanmean(f); 
...
%             tau3arraySec = tau3arrayUsec*1e-6;
            ...
            fprintf('%d/%d: Now computing the 3pt TCF of %s. Saving as %s in %s.\n',i,Nfiles,scanID,foutName,folderOutName);
      ...
      if ~strcmp(quantityName,'FRET')
        if usres==10  
%             || usres==250 - took this case out on 5/27/2022 to see if we
%             can retain full grid for stitch/no stitch plotting
%***************************************************************************
% THIS BLOCK IS FOR THE SHORT RIBBON CASE, SHOULD ADD ALL INTENDED FUTURE
% RESOLUTIONS TO THE BLOCK BELOW
%**********************************************************************
...
%             calulate the FIRST "side"
    ...
    if ~strcmp(quantityName,'FRET')   
%         NOTE: 5/27/2022 - added 250us to the full Tau1L x Tau3L case
%         since Protein data appears to be achieveable at c4=250usec.
%         Stitch routine shouldnt be affected by this, although the slow
%         down of calcualting the full 250usec surface should be considered
     if usres==250 || usres==1000 || usres==10000 || usres==500 || usres==5000 || usres==100 || usres==50
          %--------------------------------------------------------------------------
          % Vary Tau 1
          %--------------------------------------------------------------------------
...
%                 foutName = [scanID '_' num2str(usres,'%06d') 'us_tau2eq' num2str(tau2ValUsec,'%06d') 'us_fourptTCF.mat'];
           ...
           avgFileRunTime=avgFileRunTime/NfilesToIterate; 
           totalRunTime= toc
end 