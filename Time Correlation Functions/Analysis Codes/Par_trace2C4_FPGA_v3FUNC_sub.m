...
% Program information
%--------------------------------------------------------------------------
function [totalRunTime,avgFileRunTime,tau1L,tau3L,NfilesToIterate,filesList] = Par_trace2C4_FPGA_v3FUNC_sub(usres,constructName,tau2ArrayUnits_us,subsetOpt,subsetLoc,subsetName,quantityName) 
...
%--------------------------------------------------------------------------
    ...
    if subsetOpt
    folderOutName = ['4ptTCFs_v2_SLsub' subsetName '_' constructName  filesep() tau2_subFolderName];
    else
    folderOutName = ['4ptTCFs_v2_SL_' constructName  filesep() tau2_subFolderName];
    end
...
%             tau3arraySec = tau3arrayUsec*1e-6;
            ...
            if quantityName=='P1PhaseFact'
            f = curFile.P1PhaseFact;
            elseif quantityName=='P1PhaseFactN'
               f = curFile.P1PhaseFactN; 
            elseif quantityName=='P1Amp'
                f = curFile.P1PhaseFact;
                f = abs(f); 
            elseif quantityName=='P1AmpAvg'
                f = curFile.P1PhaseFactN;
                f = abs(f); 
            end
            fAvg = nanmean(f);
