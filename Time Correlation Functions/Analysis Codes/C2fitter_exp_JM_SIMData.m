...
% set the y-axis limits (based on data set scaling)
yUp=1;
yBot=0.04; 
...
sample_description= ['E5E6'];
save_prefix= ['E5E6_' num2str(usres) 'usec']; 
...
%     tcf_max_time_to_fit_sec = .001;
    tcf_max_time_to_fit_sec = 3;  % Only fit the data up to 1s
elseif usres == 1
    tcf_min_time_to_fit_sec = 2e-6;
    tcf_max_time_to_fit_sec = 1;
elseif usres == 1000
...
if stitchFit~=1
load(tcfavg_filename,'tauArraySec','pairsTcfFinal','numMolecules','totalInList');
...
if fit_1_exp_mode
    ...
    opts.Lower =      [A1_LB  t1_LB  yoff_LB];
    opts.Upper =      [A1_UB  t1_UB  yoff_UB];
    ...
    for i = 1:maxTrials
        ...
        opts.StartPoint = [A1_SPm t1_SPm  yoff_SPm];
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Do the Fit
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        fprintf('%d/%d fitresult #%d: A1 = %f, t1 = %f, yoff = %f\n',i,maxTrials,iterNumber,A1,t1,yoff);
    end
...
%**************************************************************************
if fit_2_exp_mode
    fitFunction = 'A1*exp(-x/t1) + A2*exp(-x/t2) + yoff';
    %----------------------------------------------------------------------
    ...
    t1_LB = tcf_min_time_to_fit_sec;
    t1_UB = 10*tcf_min_time_to_fit_sec;  %t1_UB = abs(t1_UB + .2*randn);
    t1_SP = (t1_LB + t1_UB)/2;
    ...
    yoff_SP = yData(end);
    %*** THESE MUT BE IN ALPHABETICAL ORDER ***
    opts.Lower =      [A1_LB A2_LB t1_LB t2_LB yoff_LB];
    %        opts.StartPoint = [A1_SP A2_SP t1_SP t2_SP yoff_SP];
    opts.Upper =      [A1_UB A2_UB t1_UB t2_UB yoff_UB];
    ...
    for i = 1:maxTrials
        ...
        opts.StartPoint = [A1_SPm t1_SPm A2_SPm t2_SPm yoff_SPm];
        ...
        y = A1*exp(-time/t1) + A2*exp(-time/t2) + yoff;
        ...
        if saveMode
            ...
            if findBestFit_mode
                if exist(filepath_fit,'file') ~= 2
                    disp('Did not find the file. Saving the initial fit');
                    iterNumber = 1;
                    save(filepath_fit,'fitresult','gof','A1','t1','A2','t2','yoff','time','y','rsquare','iterNumber');
                else
                    load(filepath_fit,'rsquare','iterNumber');
                    iterNumber = iterNumber + 1 ;
                    %                     disp(['Already found a fit. IterNumber = ' num2str(iterNumber)]);
                ...
                fprintf('%d/%d fitresult #%d: A1 = %f, t1 = %f, A2 = %f, t2 = %f, yoff = %f\n',i,maxTrials,iterNumber,A1,t1,A2,t2,yoff);
                
            else
                disp(['     Saving the fit results as ' foutName_prefix '.mat in ' outputFolderName]);
                save(filepath_fit,'fitresult','gof','A1','t1','A2','t2','yoff','time','y','rsquare','iterNumber');
                fprintf('\t fitresults: A1 = %f, t1 = %f, A2 = %f, t2 = %f, yoff = %f\n',A1,t1,A2,t2,yoff);
            end
        end
    end
    if saveMode
...
%         fig.Position = [4 442 579 550];%Left side of screen
    ...
    if plot_current_fit_mode == 1
        ...
        if labelPlotWithFit_mode
            ...
            if t2 < 1e-3
                text(t2,fitresult(t2),['{\bf\leftarrow}' num2str(t2*1e6,'%2.1f') ' \musec, Amp=' num2str(A2)],'Color','black','FontSize',14);
            else
                text(t2,fitresult(t2),['{\bf\leftarrow}' num2str(t2*1e3,'%2.1f') ' msec, Amp=' num2str(A2)],'Color','black','FontSize',14);
            end
        end
    end
...
outputFolderName = [functionName '_out' date_str special_folder_ending];
    ...
    load(filepath_fit,'fitresult','gof','A1','t1','A2','t2','yoff','time','y','rsquare','iterNumber');
...
%     whitebg;
    ...
    if labelPlotWithFit_mode
        if t1 < 1e-3
            text(t1,fitresult(t1),['{\bf\leftarrow}' num2str(t1 *1e6,'%2.1f') ' \musec, A=' num2str(A1)],'Color','black','FontSize',14);
        else
            text(t1,fitresult(t1),['{\bf\leftarrow}' num2str(t1*1e3,'%2.1f') ' msec, A=' num2str(A1)],'Color','black','FontSize',14);
        end
        if t2 < 1e-3
            text(t2,fitresult(t2),['{\bf\leftarrow}' num2str(t2*1e6,'%2.1f') ' \musec, A=' num2str(A2)],'Color','black','FontSize',14);
        else
            text(t2,fitresult(t2),['{\bf\leftarrow}' num2str(t2*1e3,'%2.1f') ' msec, A=' num2str(A2)],'Color','black','FontSize',14);
        end
    end
...
%**************************************************************************
if fit_3_exp_mode == 1
    fitFunction = 'A1*exp(-x/t1) + A2*exp(-x/t2) + A3*exp(-x/t3) + yoff';
    %----------------------------------------------------------------------
    ...
    t1_LB = tcf_min_time_to_fit_sec;
    t1_UB = 2*tcf_min_time_to_fit_sec;
%     t1_UB = 5*tcf_min_time_to_fit_sec;
    t1_SP = (t1_LB + t1_UB)/2;
...
%     t2_UB = 100*tcf_min_time_to_fit_sec;
    ...
    opts.Lower =      [A1_LB A2_LB A3_LB t1_LB t2_LB t3_LB yoff_LB];
    opts.Upper =       [A1_UB A2_UB A3_UB t1_UB t2_UB t3_UB yoff_UB];
    ...
    for i = 1:maxTrials
        ...
        opts.StartPoint = [A1_SPm t1_SPm A2_SPm t2_SPm A3_SPm t3_SPm yoff_SPm];
        %*** THESE MUT BE IN ALPHABETICAL ORDER ***
        ...
        y = A1*exp(-time/t1) + A2*exp(-time/t2) + A3*exp(-time/t3)+yoff;
        ...
        if saveMode
            ...
            if findBestFit_mode
                if exist(filepath_fit,'file') ~= 2
                    disp('Did not find the file. Saving the initial fit');
                    iterNumber = 1;
                    save(filepath_fit,'fitresult','gof','A1','t1','A2','t2','A3','t3','yoff','time','y','rsquare','iterNumber');
                else
                    load(filepath_fit,'rsquare','iterNumber');
                    iterNumber = iterNumber + 1 ;
                    %                     disp(['Already found a fit. IterNumber = ' num2str(iterNumber)]);
                ...
                fprintf('%d/%d fitresult #%d: A1 = %f, t1 = %f, A2 = %f, t2 = %f, A3 = %f, t3 = %f,yoff = %f\n',i,maxTrials,iterNumber,A1,t1,A2,t2,A3,t3,yoff);
            else
                disp(['     Saving the fit results as ' foutName_prefix '.mat in ' outputFolderName]);
                fprintf('\tfitresults: A1 = %f, t1 = %f, A2 = %f, t2= %f, A3 = %f, t3 = %f, yoff = %f\r',A1,t1,A2,t2,A3,t3,yoff);
                save(filepath_fit,'fitresult','gof','A1','t1','A2','t2','A3','t3','yoff','time','y','rsquare','iterNumber');
            end
        end
    end
...
%         fig.Position = [4 442 579 550];%Left side of screen
    ...
    load(filepath_fit,'fitresult','gof','A1','t1','A2','t2','A3','t3','yoff','time','y','rsquare','iterNumber');
...
%     whitebg;
    ...
    if labelPlotWithFit_mode
        ...
        if t3 < 1e-3
            text(t3,fitresult(t3),['{\bf\leftarrow}' num2str(t3*1e6,'%2.1f') ' \musec, A=' num2str(A3)],'Color','black','FontSize',14);
        else
            text(t3,fitresult(t3),['{\bf\leftarrow}' num2str(t3*1e3,'%2.1f') ' msec, A=' num2str(A3)],'Color','black','FontSize',14);
        end
...
if fit_4_exp_mode
    ...
    % Set up fittype and options.
    [xData, yData] = prepareCurveData( time, tcfavg );
    fitFunction = 'A1*exp(-x/t1) + A2*exp(-x/t2) + A3*exp(-x/t3) + A4*exp(-x/t4) + yoff';
    if saveMode
    ...
    t1_LB = tcf_min_time_to_fit_sec;       t1_UB = 3*tcf_min_time_to_fit_sec;     t1_SP = (t1_LB + t1_UB)/2;
    %t1_UB = 3*tcf_min_time_to_fit_sec;

    %A2 is the amplitude of the second decay, t2 is its time constant
    A2_LB = 0;       A2_UB = max(yData);    A2_SP = (A2_LB + A2_UB)/4;
    t2_LB = t1_UB;      t2_UB = 30*tcf_min_time_to_fit_sec;   t2_SP = (t2_LB + t2_UB)/2;
    % t2_UB = 50*tcf_min_time_to_fit_sec; 

    %A3 is the amplitude of the second decay, t3 is its time constant
    A3_LB = 0;       A3_UB = max(yData);       A3_SP = (A3_LB + A3_UB)/4;
    t3_LB = t2_UB;  t3_UB = 3000*tcf_min_time_to_fit_sec;   t3_SP = (t3_LB + t3_UB)/2;
...
%     t4_LB = 1000*tcf_min_time_to_fit_sec;
    ...
    %*** THESE MUT BE IN ALPHABETICAL ORDER ***
    opts.Lower =      [A1_LB  A2_LB A3_LB A4_LB t1_LB t2_LB t3_LB t4_LB yoff_LB];
    
    %     if exist('A1_SP_ng','var')
    %         disp('$$$$Feeding results back in$$$$$');
    %         opts.StartPoint =  [A1_SP_ng A2_SP_ng A3_SP_ng A4_SP_ng t1_SP_ng t2_SP_ng t3_SP_ng t4_SP_ng yoff_SP_ng];
    %     end
    opts.Upper =       [A1_UB A2_UB A3_UB A4_UB t1_UB t2_UB t3_UB t4_UB yoff_UB];
    
    randmult = 10;
    for i = 1:maxTrials % Loop to do many fits
        opts.StartPoint =  [A1_SP A2_SP A3_SP A4_SP t1_SP t2_SP t3_SP t4_SP yoff_SP]*randn*randmult;
        disp(['i = ' num2str(i) '/' num2str(maxTrials)]);
        [fitresult, gof] = fit( xData, yData, ft, opts );
        rsquare = gof.rsquare;
        rsquareNew = rsquare;
        ...
        y = A1*exp(-time/t1) + A2*exp(-time/t2) + A3*exp(-time/t3) + A4*exp(-time/t4) + yoff;
        
        fprintf('\tfitresults: A1 = %f, t1 = %f, A2 = %f, t2= %f, A3 = %f, t3 = %f, A4 = %f, t4 = %f,  yoff = %f\r\n',A1,t1,A2,t2,A3,t3,A4,t4,yoff);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % NEW JUNE 9 2019 randomized global optimization
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if saveMode
            ...
            if findBestFit_mode
                if exist(filepath_fit,'file') ~= 2
                    disp('Did not find the file. Saving the initial fit');
                    iterNumber = 1;
                    save(filepath_fit,'fitresult','gof','A1','t1','A2','t2','A3','t3','A4','t4','yoff','time','y','rsquare','iterNumber');
                else
                    load(filepath_fit,'rsquare','iterNumber');
                    iterNumber = iterNumber + 1 ;
                    %                     disp(['Already found a fit. IterNumber = ' num2str(iterNumber)]);
...
%     whitebg;
    ...
    if labelPlotWithFit_mode
        ...
        if t3 < 1e-3
            text(t3,fitresult(t3),['{\bf\leftarrow}' num2str(t3*1e6,'%2.1f') ' \musec, Amp=' num2str(A3)],'Color','black','FontSize',14);
        else
            text(t3,fitresult(t3),['{\bf\leftarrow}' num2str(t3*1e3,'%2.1f') ' msec, Amp=' num2str(A3)],'Color','black','FontSize',14);
        end
        
        if t4 < 1e-3
            text(t4,fitresult(t4),['{\bf\leftarrow}' num2str(t4*1e6,'%2.1f') ' \musec, Amp=' num2str(A4)],'Color','black','FontSize',14);
        elseif t4 < 1
            text(t4,fitresult(t4),['{\bf\leftarrow}' num2str(t4*1e3,'%2.1f') ' msec, Amp=' num2str(A4)],'Color','black','FontSize',14);
        else
            text(t4,fitresult(t4),['{\bf\leftarrow}' num2str(t4,'%2.1f') ' sec, Amp=' num2str(A4)],'Color','black','FontSize',14);
        end
    end
...
if fit_5_exp_mode
    ...
    %A1 is the amplitude of the first decay, t1 is the time constant
    A1_LB = 0;       A1_UB = 1.4*max(yData);     A1_SP = (A1_LB + A1_UB)/5;
    ...
    %yoff is the y-offset
    yoff_LB = 0;       yoff_UB = max(yData);       yoff_SP = (yoff_LB + yoff_UB)/2;
