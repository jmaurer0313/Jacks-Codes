...
# Values for the +15 Dimer at ED level of treatment
...
def TargetAbsCDSpectra_OLD(phiN_thetaN_R12ang_sigma_chiAbs_chiCD):
...
useTQ = True
useED = False
#myPhisIn=[65, 70, 75, 80, 85, 90, 95, 100, 105, 110]
...
for index,val in enumerate(myPhisIn):
# for val in (myPhisIn):
    phi_sm = val
    AbsPlusEnergy=[]
    AbsPlusAmp=[]
    ...
    if useED:
        ...
        print(TargetAbsCDSpectra_OLD(start))
    elif useTQ:
        #starting points from some opt fit in TQ
        start = [phi_sm,  theta_sm, 108.7, 0.5, 0.92, r_sm, sigma_sm, 0.077028, 0.37574886]
...
#######Single Molecule Visibility Calculation######################
    ...
    # pluses = (np.sum(l[0+(index*3)]) + np.sum(l[1+(index*3)]) + np.sum(l[2+(index*3)]))
    # minuses = (np.sum(m[0+(index*3)]) + np.sum(m[1+(index*3)]) + np.sum(m[2+(index*3)]))
    
    #JWM define the visiblity here. 
    A = round(abs((pluses - minuses)/(minuses + pluses)),5)
    myVisbOuts.append(A)
    myPhisOut.append(phi_sm)
    numpy_arrays = [l, m]
    ...
    plt.figure(figsize=(10,8))
    plt.plot(myPhisOutTot,myJsOut, color = 'r', linewidth= 1.75, linestyle = '--', label = '$1^{st}$ J-values')
    plt.xlabel('Phi Angle (Deg)', fontsize = 16)
    plt.ylabel('J-Value ($cm^{-1}$)', fontsize = 16)

    plt.figure(figsize=(10,8))
    plt.plot(myPhisOutTot,myVisbOutsTot, color = 'b', linewidth= 1.75, linestyle = '--', label = '$1^{st}$ J-values')
    plt.xlabel('Phi Angle (Deg)', fontsize = 16)
    plt.ylabel('Visibility', fontsize = 16)
    
    plt.figure(figsize=(10,8))
    plt.plot(myPhisOutTot,myVisbOutsTotED_ED, color = 'b', linewidth= 1.75, linestyle = '--', label = '$1^{st}$ J-values')
    plt.plot(myPhisOutTot,myVibsOutTQ_ED, color = 'r', linewidth= 1.75, linestyle = '--', label = '$1^{st}$ J-values')
    plt.plot(myPhisOutTot,myVisbOutsTotTQ_TQ, color = 'g', linewidth= 1.75, linestyle = '--', label = '$1^{st}$ J-values')
    plt.xlabel('Phi Angle (Deg)', fontsize = 16)
    plt.ylabel('Visibility', fontsize = 16)
